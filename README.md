...
YouTube チャンネルからレシピを取り込み、食材・調理時間・カテゴリで検索できる。

- YouTube Data API v3 でチャンネルの動画一覧を取得し、LLM（GLM-5）で構造化して DB に保存
- **再生回数順での取得に対応**（Search API使用）。オフセット指定で「51〜100位」等のページ送りも可能
- 概要欄の原文もそのまま保存し、エージェントが作り方を案内可能
- 食材・調理時間・カテゴリ・キーワードによる SQL 検索
- 献立提案時に在庫の食材にマッチするレシピを優先的に候補に含める
...
# YouTube Data API v3（Phase 3: レシピ取り込み）
...
│   ├── schema.ts               # テーブル定義 (inventory, meals, preferences, purchases, recipes)
│   └── seed.ts                 # 初期データ投入
├── tools/
...
| `search-recipes` | 食材・調理時間・カテゴリ・キーワードによるレシピ検索 |
| `import-recipes` | YouTubeチャンネルからレシピ動画を一括取り込み（YouTube API → LLM構造化 → DB保存）。再生回数順・オフセット指定対応 |
